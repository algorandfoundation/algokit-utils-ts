...
describe('AddressCodec', () => {
  ...
  describe('encode', () => {
    describe('default values', () => {
      test.each<{ value: string | undefined; description: string }>([
        ...
        expect(addressCodec.encode(value, 'json')).toBeUndefined()
        expect(addressCodec.encode(value, 'msgpack')).toBeUndefined()
      })
    })
  ...
  describe('decode', () => {
    ...
    describe('from Uint8Array', () => {
      test.each<{ address: string; description: string }>([
        ...
        const bytes = addressCodec.encode(address, 'msgpack') as Uint8Array
      ...
      test('should decode known byte arrays to expected addresses', () => {
        // address1: VCMJKWOY5P5P7SKMZFFOCEROPJCZOTIJMNIYNUCKH7LRO45JMJP6UYBIJA
        const bytes1 = new Uint8Array([
          168, 152, 149, 89, 216, 235, 250, 255, 201, 76, 201, 74, 225, 18, 46, 122, 69, 151, 77, 9, 99, 81, 134, 208, 74, 63, 215, 23, 115,
          169, 98, 95,
        ])
        expect(addressCodec.decode(bytes1, 'msgpack')).toBe(VALID_ADDRESSES.address1)
      })
    })
  ...
  describe('decodeOptional', () => {
    test('should preserve undefined', () => {
      expect(addressCodec.decodeOptional(undefined, 'json')).toBeUndefined()
      expect(addressCodec.decodeOptional(undefined, 'msgpack')).toBeUndefined()
    })
    ...
    test('should decode Uint8Array', () => {
      const bytes = addressCodec.encode(VALID_ADDRESSES.address1, 'msgpack') as Uint8Array
  ...
  describe('error handling', () => {
    ...
    test('should throw error on invalid address string format', () => {
      ...
      expect(() => addressCodec.encode(invalidAddress, 'msgpack')).toThrow()
    })
  })
})
