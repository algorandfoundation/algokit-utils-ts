...
export async function getWalletHandle(
  ...
  const initResult = await client.initWalletHandleToken({
    walletId,
    walletPassword: password,
  })
...
/**
 * Creates a test transaction blob for signing tests
 * Note: Requires algod client for suggested params - will be implemented when needed
 */
export async function createTestTransaction(from: string, to: string): Promise<Uint8Array> {
  // TODO: Implement when transaction signing tests are added
  // Will need algod client to get suggested params
  throw new Error('Not implemented yet - will be added for transaction signing tests')
}

/**
 * Gets a compiled test program blob for program signing tests
 * Note: Requires algod client for compilation - will be implemented when needed
 */
export async function getTestProgram(): Promise<Uint8Array> {
  // TODO: Implement when program signing tests are added
  // Will need algod client to compile TEAL source
  throw new Error('Not implemented yet - will be added for program signing tests')
}