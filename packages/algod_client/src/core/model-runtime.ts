...
import { ModelSerializer, type FieldMetadata, type BodyFormat, type ModelMetadata } from '@algorandfoundation/algokit-common'

// Re-export types for convenience
export type { BodyFormat, FieldMetadata, ModelMetadata }
...
export class AlgorandSerializer {
  ...
  static encode(
    ...
    if (format === 'map') {
      ...
      const wire = ModelSerializer.encode(value as object, meta, 'msgpack')
      return this.convertToNestedMaps(wire) as Map<string, unknown>
    }

    const wire = ModelSerializer.encode(value as object, meta, format)
    if (format === 'msgpack') {
