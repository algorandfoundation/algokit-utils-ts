import { ABIType, getABIType, getABITypeName, parseTupleContent } from '@algorandfoundation/algokit-abi'
...
export class ABIMethod {
  ...
  constructor(params: ABIMethodParams) {
    ...
    this.returns = {
      type: params.returns.type === 'void' ? params.returns.type : getABIType(params.returns.type),
  ...
  getSignature(): string {
    const args = this.args.map((arg) => (typeof arg.type === 'string' ? arg.type : getABITypeName(arg.type))).join(',')
    const returns = typeof this.returns.type === 'string' ? this.returns.type : getABITypeName(this.returns.type)
  ...
  toJSON(): ABIMethodParams {
    return {
      ...
      args: this.args.map(({ type, name, description }) => ({
        type: typeof type === 'string' ? type : getABITypeName(type),
        name,
        desc: description,
      })),
      returns: {
        type: typeof this.returns.type === 'string' ? this.returns.type : getABITypeName(this.returns.type),
...
export function getMethodByName(methods: ABIMethod[], name: string): ABIMethod {
  ...
  if (filteredMethods.length > 1)
    throw new Error(
      `found ${filteredMethods.length} methods with the same name ${filteredMethods.map((m: ABIMethod) => m.getSignature()).join(',')}`,
    )
