import { BaseHTTPClient, BaseHTTPClientResponse, BaseHTTPClientError, Query } from './baseHTTPClient.js'
...
export class URLTokenBaseHTTPClient implements BaseHTTPClient {
  ...
  private static async checkHttpError(res: Response) {
    ...
    try {
      ...
      const decoded: Record<string, any> = JSON.parse(new TextDecoder().decode(body))
      if (decoded.message) {
