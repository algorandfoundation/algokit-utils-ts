...
The core internal type that holds information about a signer/sender pair for a transaction is [`TransactionSignerAccount`](../../api/types/account/type-aliases/TransactionSignerAccount.md), which represents an `algosdk.TransactionSigner` (`signer`) along with a sender address (`addr`) as the encoded string address.
...
If you want to have a default signer that is used to sign transactions without a registered signer (rather than throwing an exception) then you can [register a default signer](../../api/types/account-manager/classes/AccountManager.md#setdefaultsigner):
...
[`AlgorandClient`](./algorand-client.md) will automatically retrieve a signer when signing a transaction, but if you need to get a `TransactionSigner` externally to do something more custom then you can [retrieve the signer](../../api/types/account-manager/classes/AccountManager.md#getsigner) for a given sender address:
...
- [`algorand.account.fromMnemonic(mnemonicSecret, sender?)`](../../api/types/account-manager/classes/AccountManager.md#frommnemonic) - Registers and returns an account with secret key loaded by taking the mnemonic secret
- [`algorand.account.multisig(multisigParams, signingAccounts)`](../../api/types/account-manager/classes/AccountManager.md#multisig) - Registers and returns a multisig account with one or more signing keys loaded
- [`algorand.account.rekeyed(sender, signer)`](../../api/types/account-manager/classes/AccountManager.md#rekeyed) - Registers and returns an account representing the given rekeyed sender/signer combination
- [`algorand.account.random()`](../../api/types/account-manager/classes/AccountManager.md#random) - Returns a new, cryptographically randomly generated account with private key loaded
- [`algorand.account.fromKmd()`](../../api/types/account-manager/classes/AccountManager.md#fromkmd) - Returns an account with private key loaded from the given KMD wallet (identified by name)
- [`algorand.account.logicsig(program, args?)`](../../api/types/account-manager/classes/AccountManager.md#logicsig) - Returns an account that represents a logic signature
...
- [`MultisigAccount`](../../api/Subpaths/transact/classes/MultisigAccount.md) - An abstraction around `algosdk.MultisigMetadata`, `algosdk.makeMultiSigAccountTransactionSigner`, `algosdk.multisigAddress`, `algosdk.signMultisigTransaction` and `algosdk.appendSignMultisigTransaction` that supports multisig accounts with one or more signers present

### Dispenser

- [`algorand.account.dispenserFromEnvironment()`](../../api/types/account-manager/classes/AccountManager.md#dispenserfromenvironment) - Returns an account (with private key loaded) that can act as a dispenser from environment variables, or against default LocalNet if no environment variables present
- [`algorand.account.localNetDispenser()`](../../api/types/account-manager/classes/AccountManager.md#localnetdispenser) - Returns an account with private key loaded that can act as a dispenser for the default LocalNet dispenser account
...
You can issue a transaction to rekey an account by using the [`algorand.account.rekeyAccount(account, rekeyTo, options)`](../../api/types/account-manager/classes/AccountManager.md#rekeyaccount) function:
...
- [`getWalletAccount(walletName, predicate?, sender?)`](../../api/types/kmd-account-manager/classes/KmdAccountManager.md#getwalletaccount) - Returns an Algorand signing account with private key loaded from the given KMD wallet (identified by name).
- [`getOrCreateWalletAccount(name, fundWith?)`](../../api/types/kmd-account-manager/classes/KmdAccountManager.md#getorcreatewalletaccount) - Gets an account with private key loaded from a KMD wallet of the given name, or alternatively creates one with funds in it via a KMD wallet of the given name.
- [`getLocalNetDispenserAccount()`](../../api/types/kmd-account-manager/classes/KmdAccountManager.md#getlocalnetdispenseraccount) - Returns an Algorand account with private key loaded for the default LocalNet dispenser account (that can be used to fund other accounts). The "Wallet1" KMD account is resolved from the genesis file and used as the dispenser account.
