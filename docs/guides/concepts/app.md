...
The [`AppManager`](../../api/types/app-manager/classes/AppManager.md) is a class that is used to manage app information.
...
- All app calls extend [`SendAppTransactionResult`](../../api/types/app/type-aliases/SendAppTransactionResult.md), which has:
...
- Update and create calls extend [`SendAppUpdateTransactionResult`](../../api/types/app/type-aliases/SendAppUpdateTransactionResult.md), which has:
...
- Create calls extend [`SendAppCreateTransactionResult`](../../api/types/app/type-aliases/SendAppCreateTransactionResult.md), which has:
...
The base type for specifying an app creation transaction is [`AppCreateParams`](../../api/types/composer/type-aliases/AppCreateParams.md) (extended as [`AppCreateMethodCall`](../../api/types/composer/type-aliases/AppCreateMethodCall.md) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
If you pass in `approvalProgram` or `clearStateProgram` as a string then it will automatically be compiled using Algod and the compilation result will be available via [`algorand.app.getCompilationResult`](../../api/types/app-manager/classes/AppManager.md#getcompilationresult) (including the source map). To skip this behaviour you can pass in the compiled TEAL as `Uint8Array`.
...
The base type for specifying an app update transaction is [`AppUpdateParams`](../../api/types/composer/type-aliases/AppUpdateParams.md) (extended as [`AppUpdateMethodCall`](../../api/types/composer/type-aliases/AppUpdateMethodCall.md) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
The base type for specifying an app deletion transaction is [`AppDeleteParams`](../../api/types/composer/type-aliases/AppDeleteParams.md) (extended as [`AppDeleteMethodCall`](../../api/types/composer/type-aliases/AppDeleteMethodCall.md) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
The base type for specifying an app call transaction is [`AppCallParams`](../../api/types/composer/type-aliases/AppCallParams.md) (extended as [`AppCallMethodCall`](../../api/types/composer/type-aliases/AppCallMethodCall.md) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
- [`algorand.app.getLocalState(appId, address)`](../../api/types/app-manager/classes/AppManager.md#getlocalstate) - Returns the current local state for the given app ID and account address decoded into an object keyed by the UTF-8 representation of the state key with various parsed versions of the value (base64, UTF-8 and raw binary)
...
- [`AppManager.decodeAppState(state)`](../../api/types/app-manager/classes/AppManager.md#decodeappstate) - Takes the raw response from the algod API for global state and returns a friendly generic object keyed by the UTF-8 value of the key
...
- [`algorand.app.getBoxNames(appId: bigint)`](../../api/types/app-manager/classes/AppManager.md#getboxnames) - Returns the current [box names](#boxname) for the given app ID
- [`algorand.app.getBoxValue(appId: bigint, boxName: BoxIdentifier)`](../../api/types/app-manager/classes/AppManager.md#getboxvalue) - Returns the binary value of the given box name for the given app ID
- [`algorand.app.getBoxValues(appId: bigint, boxNames: BoxIdentifier[])`](../../api/types/app-manager/classes/AppManager.md#getboxvalues) - Returns the binary values of the given box names for the given app ID
- [`algorand.app.getBoxValueFromABIType(request: {appId: bigint, boxName: BoxIdentifier, type: algosdk.ABIType}})`](../../api/types/app-manager/classes/AppManager.md#getboxvaluefromabitype) - Returns the parsed ABI value of the given box name for the given app ID for the provided ABI type
- [`algorand.app.getBoxValuesFromABIType(request: {appId: bigint, boxNames: BoxIdentifier[], type: algosdk.ABIType})`](../../api/types/app-manager/classes/AppManager.md#getboxvaluesfromabitype) - Returns the parsed ABI values of the given box names for the given app ID for the provided ABI type
- [`AppManager.getBoxReference(boxId)`](../../api/types/app-manager/classes/AppManager.md#getboxreference) - Returns a `algosdk.BoxReference` representation of the given [box identifier / reference](#box-references), which is useful when constructing a raw `algosdk.Transaction`
...
- [`algorand.app.getById(appId)`](../../api/types/app-manager/classes/AppManager.md#getbyid) - Returns current app information by app ID from an [`AppManager`](#appmanager) instance
...
When interacting with apps (creating, updating, deleting, calling), there are some [`CommonAppCallParams`](../../api/types/composer/type-aliases/CommonAppCallParams.md) that you will be able to pass in to all calls in addition to the [common transaction parameters](./algorand-client.md#transaction-parameters):
...
When making an ABI call, the `args` parameter is replaced with a different type and there is also a `method` parameter per the [`AppMethodCall`](../../api/types/composer/type-aliases/AppMethodCall.md) type:
