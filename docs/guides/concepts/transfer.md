...
The base type for specifying a payment transaction is [`PaymentParams`](../../api/types/composer/type-aliases/PaymentParams.md), which has the following parameters in addition to the [common transaction parameters](./algorand-client.md#transaction-parameters):
...
All 3 variants return an `EnsureFundedReturnType` (and the first two also return a [single transaction result](./algorand-client.md#sending-a-single-transaction)) if a funding transaction was needed, or `undefined` if no transaction was required:
