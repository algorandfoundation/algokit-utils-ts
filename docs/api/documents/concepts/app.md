...
[@algorandfoundation/algokit-utils](../../modules.md) / concepts/app
...
The [`AppManager`](../../_media/AppManager.md) is a class that is used to manage app information.
...
When [sending transactions directly via AlgorandClient](algorand-client.md) the `SingleSendTransactionResult` return value is expanded with extra fields depending on the type of app call:

- All app calls extend [`SendAppTransactionResult`](../../_media/SendAppTransactionResult.md), which has:
...
- Update and create calls extend [`SendAppUpdateTransactionResult`](../../_media/SendAppUpdateTransactionResult.md), which has:
...
- Create calls extend [`SendAppCreateTransactionResult`](../../_media/SendAppCreateTransactionResult.md), which has:
...
The base type for specifying an app creation transaction is [`AppCreateParams`](../../_media/AppCreateParams.md) (extended as [`AppCreateMethodCall`](../../_media/AppCreateMethodCall.md) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
If you pass in `approvalProgram` or `clearStateProgram` as a string then it will automatically be compiled using Algod and the compilation result will be available via [`algorand.app.getCompilationResult`](../../_media/AppManager.md#getcompilationresult) (including the source map). To skip this behaviour you can pass in the compiled TEAL as `Uint8Array`.
...
The base type for specifying an app update transaction is [`AppUpdateParams`](../../_media/AppUpdateParams.md) (extended as [`AppUpdateMethodCall`](../../_media/AppUpdateMethodCall.md) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
The base type for specifying an app deletion transaction is [`AppDeleteParams`](../../_media/AppDeleteParams.md) (extended as [`AppDeleteMethodCall`](../../_media/AppDeleteMethodCall.md) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
The base type for specifying an app call transaction is [`AppCallParams`](../../_media/AppCallParams.md) (extended as [`AppCallMethodCall`](../../_media/AppCallMethodCall.md) for ABI method call version), which has the following parameters in addition to the [common parameters](#common-app-parameters):
...
- [`algorand.app.getLocalState(appId, address)`](../../_media/AppManager.md#getlocalstate) - Returns the current local state for the given app ID and account address decoded into an object keyed by the UTF-8 representation of the state key with various parsed versions of the value (base64, UTF-8 and raw binary)
...
- [`AppManager.decodeAppState(state)`](../../_media/AppManager.md#decodeappstate) - Takes the raw response from the algod API for global state and returns a friendly generic object keyed by the UTF-8 value of the key
...
- [`algorand.app.getBoxNames(appId: bigint)`](../../_media/AppManager.md#getboxnames) - Returns the current [box names](#boxname) for the given app ID
- [`algorand.app.getBoxValue(appId: bigint, boxName: BoxIdentifier)`](../../_media/AppManager.md#getboxvalue) - Returns the binary value of the given box name for the given app ID
- [`algorand.app.getBoxValues(appId: bigint, boxNames: BoxIdentifier[])`](../../_media/AppManager.md#getboxvalues) - Returns the binary values of the given box names for the given app ID
- [`algorand.app.getBoxValueFromABIType(request: {appId: bigint, boxName: BoxIdentifier, type: algosdk.ABIType}})`](../../_media/AppManager.md#getboxvaluefromabitype) - Returns the parsed ABI value of the given box name for the given app ID for the provided ABI type
- [`algorand.app.getBoxValuesFromABIType(request: {appId: bigint, boxNames: BoxIdentifier[], type: algosdk.ABIType})`](../../_media/AppManager.md#getboxvaluesfromabitype) - Returns the parsed ABI values of the given box names for the given app ID for the provided ABI type
- [`AppManager.getBoxReference(boxId)`](../../_media/AppManager.md#getboxreference) - Returns a `algosdk.BoxReference` representation of the given [box identifier / reference](#box-references), which is useful when constructing a raw `algosdk.Transaction`
...
When making an ABI call, the `args` parameter is replaced with a different type and there is also a `method` parameter per the [`AppMethodCall`](../../_media/AppMethodCall.md) type:
...
- `args: ABIArgument[]` The arguments to pass to the ABI call, which can be one of:
  ...
  - `algosdk.Transaction`
  - `Promise<Transaction>` - which allows you to use an AlgorandClient call that [returns a transaction](algorand-client.md) without needing to await the call
