...
The next step is to get an `AlgorandClient` instance at the same place(s) you had an algod instance. To do this you can look for anywhere you called the `getAlgoClient` method and replace them with an [equivalent mechanism](../concepts/algorand-client.md) for getting an `AlgorandClient` instance.
...
- We now restrict the number of valid rounds (10 rounds, except when pointing to LocalNet, which is still 1000 to avoid problems given the round advances for every transaction) to a much smaller window than the default (1000 rounds), but this is configurable [by default](../concepts/algorand-client.md) and [per transaction](../concepts/algorand-client.md) if that's a problem. If you come across this problem it will present as a dead transaction error.
- Transaction parameters are cached for a period of time to prevent repeated calls to the algod API to get default transaction parameters, but this sometimes means that you can create duplicate transaction IDs when previously that wouldn't happen, you will get an obvious error if that happens though and can adjust it by ensuring one of the parameters in your transaction change slightly (e.g. note, lease, validity window, etc.), you can also [exert control over default transaction parameter caching](../concepts/algorand-client.md)
- Rather than always passing a signer into transaction calls (which is what the `SendTransactionFrom` instance previously combined with the address), we have decoupled signing and sender address via the `AccountManager` (`algorand.account`), which keeps track of the signer associated with a sender address so the signer can be resolved just in time.
  - Most of the time you don't need to worry about it since it will magically happen for you, but if you have situations where you are creating a signer outside of the `AccountManager` you will need to [register the signer](../concepts/account.md) with the `AccountManager` first.
  - Note: you can also explicitly [pass a `signer`](../concepts/algorand-client.md) in as well if you want an escape hatch.
...
- All of the methods that return or execute a transaction (`update`, `call`, `optIn`, etc.) are now exposed in an interface similar to the one in [`AlgorandClient`](../concepts/algorand-client.md), namely (where `{callType}` is one of: `update` / `delete` / `optIn` / `closeOut` / `clearState` / `call`):
  ...
  - `appClient.params.{callType}` to get a [params object](../concepts/algorand-client.md) for an ABI method call
  ...
  - `appClient.params.bare.{callType}` to get a [params object](../concepts/algorand-client.md) for a bare app call
- The `resolveBy` functionality has disappeared in favour of [much simpler entrypoints within `algorand.client`](../concepts/app-client.md)
