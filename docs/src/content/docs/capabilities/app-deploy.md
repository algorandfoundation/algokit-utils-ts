...
- `updateParams.allowStateShrinking?: boolean` - allows an update to reduce the global schema or inferred extra program pages. By default, deployment rejects these reductions. The deployer infers the desired global schema from `createParams.schema` unless `updateParams.resize.schema` is provided explicitly.
...
- `onSchemaBreak?: 'replace' | 'fail' | 'append' | OnSchemaBreak` - determines what should happen if a breaking local-schema change is detected (e.g. if you need more local state than was originally requested when the contract was created). To update global schema or extra program pages in place, use `onUpdate: 'update'`; the desired global schema is inferred from `createParams.schema`, while `updateParams.resize` can override it.
