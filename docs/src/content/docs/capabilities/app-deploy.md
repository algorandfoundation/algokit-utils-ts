...
- `onSchemaBreak?: 'replace' | 'fail' | 'append' | OnSchemaBreak` - determines what should happen if a breaking local-schema change is detected (e.g. if you need more local state than was originally requested when the contract was created). To update global schema or extra program pages in place, set `updateParams.schema` or `updateParams.extraProgramPages` and use `onUpdate: 'update'`.
