...
To get an instance of `AppFactory` you can use either [`AlgorandClient`](/algokit-utils-ts/capabilities/algorand-client/) via `algorand.client.getAppFactory` or instantiate it directly (passing in an app spec, an `AlgorandClient` instance and other optional parameters):
...
To get an instance of `AppClient` you can use either [`AlgorandClient`](/algokit-utils-ts/capabilities/algorand-client/) via `algorand.client.getAppClient*` or instantiate it directly (passing in an app ID, app spec, `AlgorandClient` instance and other optional parameters):
...
- `factory.create(params?)` - Signs and sends a transaction to create an app and returns the [result of that call](/algokit-utils-ts/capabilities/app/#creation) and an [`AppClient`](#appclient) instance for the created app
- `factory.deploy(params)` - Uses the [creator address and app name pattern](/algokit-utils-ts/capabilities/app-deploy/#lookup-deployed-apps-by-name) to find if the app has already been deployed or not and either creates, updates or replaces that app based on the [deployment rules](/algokit-utils-ts/capabilities/app-deploy/#performing-a-deployment) (i.e. it's an idempotent deployment) and returns the [result of the deployment](/algokit-utils-ts/capabilities/app-deploy/#return-value) and an [`AppClient`](#appclient) instance for the created/updated/existing app

### Create

The create method is a wrapper over the `appCreate` (bare calls) and `appCreateMethodCall` (ABI method calls) [methods](/algokit-utils-ts/capabilities/app/#creation), with the following differences:
...
If you want to construct a custom create call, use the underlying [`algorand.send.appCreate` / `algorand.createTransaction.appCreate` / `algorand.send.appCreateMethodCall` / `algorand.createTransaction.appCreateMethodCall` methods](/algokit-utils-ts/capabilities/app/#creation) then you can get params objects:

- `factory.params.create(params)` - ABI method create call for deploy method or an underlying [`appCreateMethodCall` call](/algokit-utils-ts/capabilities/app/#creation)
- `factory.params.bare.create(params)` - Bare create call for deploy method or an underlying [`appCreate` call](/algokit-utils-ts/capabilities/app/#creation)

### Deploy

The deploy method is a wrapper over the [`AppDeployer`'s `deploy` method](/algokit-utils-ts/capabilities/app-deploy/#performing-a-deployment), with the following differences:
...
If you want to construct a custom deploy call, use the underlying [`algorand.appDeployer.deploy` method](/algokit-utils-ts/capabilities/app-deploy/#performing-a-deployment) then you can get params objects for the `createParams`, `updateParams` and `deleteParams`:
...
Update calls also optionally take the `appSize` parameter to [resize the app](/algokit-utils-ts/capabilities/app/#resizing-an-app) as part of the update.
...
The input payload for all of these calls is the same as the [underlying app methods](/algokit-utils-ts/capabilities/app/#calling-apps) with the caveat that the `appId` is not passed in (since the `AppClient` already knows the app ID), `sender` is optional (it uses `defaultSender` from the `AppClient` constructor if it was specified) and `method` (for ABI method calls) is a string rather than an `ABIMethod` object (which can either be the method name, or if you need to disambiguate between multiple methods of the same name it can be the ABI signature).

The return payload for all of these is the same as the [underlying methods](/algokit-utils-ts/capabilities/app/#calling-apps).
...
- A [`FundAppParams`, which has the same properties as a [payment transaction](/algokit-utils-ts/capabilities/transfer/#payment) except `receiver` is not required and `sender` is optional (if not specified then it will be set to the app client's default sender if configured).
...
- `getGlobalState()` - Gets the current global state using [`algorand.app.getGlobalState`](/algokit-utils-ts/capabilities/app/#global-state)
- `getLocalState(address: string)` - Gets the current local state for the given account address using [`algorand.app.getLocalState`](/algokit-utils-ts/capabilities/app/#local-state).
- `getBoxNames()` - Gets the current box names using [`algorand.app.getBoxNames`](/algokit-utils-ts/capabilities/app/#boxes)
- `getBoxValue(name)` - Gets the current value of the given box using [`algorand.app.getBoxValue`](/algokit-utils-ts/capabilities/app/#boxes)
- `getBoxValueFromABIType(name)` - Gets the current value of the given box from an ABI type using [`algorand.app.getBoxValueFromABIType`](/algokit-utils-ts/capabilities/app/#boxes)
- `getBoxValues(filter)` - Gets the current values of the boxes using [`algorand.app.getBoxValues`](/algokit-utils-ts/capabilities/app/#boxes)
- `getBoxValuesFromABIType(type, filter)` - Gets the current values of the boxes from an ABI type using [`algorand.app.getBoxValuesFromABIType`](/algokit-utils-ts/capabilities/app/#boxes)
...
When this debug flag is set, it will also emit debugging symbols to allow break-point debugging of the calls if the [project root is also configured](/algokit-utils-ts/capabilities/debugging/).
