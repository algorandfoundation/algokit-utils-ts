...
- `testAccountFunding?: AlgoAmount` - The [amount](/algokit-utils-ts/capabilities/amount/) of funds to allocate to the default testing account, if not specified then it will get `10` ALGO
...
- `algorand: AlgorandClient` - An [`AlgorandClient`](/algokit-utils-ts/capabilities/algorand-client/) instance
...
The key when generating a test account is getting hold of a [dispenser](/algokit-utils-ts/capabilities/transfer/#dispenser) and then [ensuring the test account is funded](/algokit-utils-ts/capabilities/transfer/#ensurefunded).

To make it easier to quickly get a test account the testing capability provides the following mechanisms:

- `algotesting.getTestAccount(testAccountParams, algod, kmd?)` - Generates a random new account, logs the mnemonic of the account (unless suppressed), funds it from the [dispenser](/algokit-utils-ts/capabilities/transfer/#dispenser)
