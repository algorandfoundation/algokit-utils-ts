...
The entry point to the vast majority of functionality in AlgoKit Utils is now available via a single entry-point, the [`AlgorandClient` class](/algokit-utils-ts/concepts/algorand-client/).
...
The next step is to get an `AlgorandClient` instance at the same place(s) you had an algod instance. To do this you can look for anywhere you called the `getAlgoClient` method and replace them with an [equivalent mechanism](/algokit-utils-ts/concepts/algorand-client/) for getting an `AlgorandClient` instance.
...
- We now restrict the number of valid rounds (10 rounds, except when pointing to LocalNet, which is still 1000 to avoid problems given the round advances for every transaction) to a much smaller window than the default (1000 rounds), but this is configurable [by default](/algokit-utils-ts/concepts/algorand-client/#tune-network-defaults-and-caching) and [per transaction](/algokit-utils-ts/concepts/transactions/#send-parameters) if that's a problem. If you come across this problem it will present as a dead transaction error.
- Transaction parameters are cached for a period of time to prevent repeated calls to the algod API to get default transaction parameters, but this sometimes means that you can create duplicate transaction IDs when previously that wouldn't happen, you will get an obvious error if that happens though and can adjust it by ensuring one of the parameters in your transaction change slightly (e.g. note, lease, validity window, etc.), you can also [exert control over default transaction parameter caching](/algokit-utils-ts/concepts/algorand-client/#tune-network-defaults-and-caching)
- Rather than always passing a signer into transaction calls (which is what the `SendTransactionFrom` instance previously combined with the address), we have decoupled signing and sender address via the `AccountManager` (`algorand.account`), which keeps track of the signer associated with a sender address so the signer can be resolved just in time.
  - Most of the time you don't need to worry about it since it will magically happen for you, but if you have situations where you are creating a signer outside of the `AccountManager` you will need to [register the signer](/algokit-utils-ts/concepts/account/#keys--signing) with the `AccountManager` first.
  - Note: you can also explicitly [pass a `signer`](/algokit-utils-ts/concepts/transactions/#send-parameters) in as well if you want an escape hatch.
...
The existing `ApplicationClient` (untyped app client) class is still present until at least v9, but it's worthwhile migrating to the new [`AppClient` and `AppFactory` classes](/algokit-utils-ts/api/types/app-client/classes/appclient/). These new clients are [ARC-56](https://github.com/algorandfoundation/ARCs/pull/258) compatible, but also support [ARC-32](https://github.com/algorandfoundation/ARCs/blob/main/ARCs/arc-0032.md) app specs and will continue to support this indefinitely until such time the community deems they are deprecated.
...
- All of the methods that return or execute a transaction (`update`, `call`, `optIn`, etc.) are now exposed in an interface similar to the one in [`AlgorandClient`](/algokit-utils-ts/concepts/transactions/#issue-a-transaction), namely (where `{callType}` is one of: `update` / `delete` / `optIn` / `closeOut` / `clearState` / `call`):
  ...
  - `appClient.params.{callType}` to get a [params object](/algokit-utils-ts/concepts/transactions/#send-parameters) for an ABI method call
  ...
  - `appClient.params.bare.{callType}` to get a [params object](/algokit-utils-ts/concepts/transactions/#send-parameters) for a bare app call
- The `resolveBy` functionality has disappeared in favour of [much simpler entrypoints within `algorand.client`](/algokit-utils-ts/api/types/app-client/classes/appclient/)
