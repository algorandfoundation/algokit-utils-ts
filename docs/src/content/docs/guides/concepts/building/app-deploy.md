...
App deployment is a higher-order use case capability provided by AlgoKit Utils that builds on top of the core capabilities, particularly [App management](/algokit-utils-ts/guides/concepts/building/app/).
...
To get an instance of `AppDeployer` you can use either [`AlgorandClient`](/algokit-utils-ts/guides/concepts/core/algorand-client/) via `algorand.appDeployer` or instantiate it directly (passing in an [`AppManager`](/algokit-utils-ts/guides/concepts/building/app/#appmanager), [`AlgorandClientTransactionSender`](/algokit-utils-ts/guides/concepts/core/algorand-client/#sending-a-single-transaction) and optionally an indexer client instance):
...
- `createParams: AppCreateParams | AppCreateMethodCall` - the parameters for an [app creation call](/algokit-utils-ts/guides/concepts/building/app/#creation) (raw or ABI method call)
- `updateParams: Omit<AppUpdateParams | AppUpdateMethodCall, 'appId' | 'approvalProgram' | 'clearStateProgram'>` - the parameters for an [app update call](/algokit-utils-ts/guides/concepts/building/app/#updating) (raw or ABI method call) without the `appId`, `approvalProgram` or `clearStateProgram`, since these are calculated by the `deploy` method
- `deleteParams: Omit<AppDeleteParams | AppDeleteMethodCall, 'appId'>` - the parameters for an [app delete call](/algokit-utils-ts/guides/concepts/building/app/#deleting) (raw or ABI method call) without the `appId`, since this is calculated by the `deploy` method
...
- Everything from `SendParams` - [transaction execution control parameters](/algokit-utils-ts/guides/concepts/core/algorand-client/#transaction-parameters)
...
- If `create`, `update` or `replace` then it will have the relevant [`SendAppTransactionResult`](/algokit-utils-ts/guides/concepts/building/app/#calling-an-app) values
- If `replace` then it will also have `{deleteReturn?: ABIReturn, deleteResult: ConfirmedTransactionResult}` to capture the [result](/algokit-utils-ts/guides/concepts/core/algorand-client/#sending-a-single-transaction) of the deletion of the existing app
