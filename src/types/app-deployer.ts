...
export interface AppDeployParams {
  ...
  updateParams: Expand<Omit<AppUpdateParams, 'appId'>> | Expand<Omit<AppUpdateMethodCall, 'appId'>>
...
export class AppDeployer {
  ...
  async deploy(deployment: AppDeployParams): Promise<AppDeployResult> {
    ...
    createParams.note = AlgoKitComposer.arc2Note({
    ...
    const compiledApproval =
      typeof createParams.approvalProgram === 'string'
        ...
        : undefined
    createParams.approvalProgram = compiledApproval ? compiledApproval.compiledBase64ToBytes : createParams.approvalProgram
    updateParams.approvalProgram = createParams.approvalProgram
    ...
    const compiledClear =
      typeof createParams.clearStateProgram === 'string'
        ...
        : undefined
    createParams.clearStateProgram = compiledClear ? compiledClear.compiledBase64ToBytes : createParams.clearStateProgram
    updateParams.clearStateProgram = createParams.clearStateProgram

    // Define routines for create, update, and replace

    const createApp = async () => {
      const result = await ('method' in createParams
        ? this._transactionSender.appCreateMethodCall({ ...createParams, ...executeParams })
        : this._transactionSender.appCreate({ ...createParams, ...executeParams }))
      return {
    ...
    const updateApp = async (existingApp: AppMetadata) => {
      ...
      const result = await ('method' in updateParams
        ? this._transactionSender.appUpdateMethodCall({ appId: existingApp.appId, ...updateParams, ...executeParams })
        : this._transactionSender.appUpdate({ appId: existingApp.appId, ...updateParams, ...executeParams }))
      return {
    ...
    const replaceApp = async (existingApp: AppMetadata) => {
      ...
      if ('method' in createParams) {
        composer.addAppCreateMethodCall(createParams)
      } else {
        composer.addAppCreate(createParams)
      }
    ...
    const apps = existingDeployments ?? (await this.getCreatorApps(createParams.sender))
    ...
    const newApproval = Buffer.from(createParams.approvalProgram).toString('base64')
    const newClear = Buffer.from(createParams.clearStateProgram).toString('base64')
   ...
   */
  async getCreatorApps(creatorAddress: string, ignoreCache?: boolean): Promise<AppLookup> {
