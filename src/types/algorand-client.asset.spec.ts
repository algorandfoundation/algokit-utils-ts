...
describe('Asset capability', () => {
  ...
  test('Create an asset succeeds', async () => {
    ...
    expect(assetData.manager).toBe(accounts[0])
    expect(assetData.reserve).toBe(accounts[1])
    expect(assetData.freeze).toBe(accounts[2])
    expect(assetData.clawback).toBe(accounts[3])
  })
