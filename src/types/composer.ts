...
/** The size an app update transaction can resize an app to. */
export type AppUpdateSize = {
  /** The size to resize the app to, or `undefined` (the default) to leave the app at its current size.
   *
   * Resizing requires AVM 13 (consensus v42) or later; before that an app's size is fixed at creation.
   *
   * The AVM sets an app's global state schema and extra program pages together, which is why they are specified
   * as a single value describing the app's whole new size rather than the increase. An app can only grow: the
   * global state schema can't be reduced below the state the app has already stored, the app can't be given
   * fewer pages than its programs need, and the local state schema stays immutable. */
  appSize?: {
    /** The number of integers saved in global state. */
    globalInts: number
    /** The number of byte slices saved in global state. */
    globalByteSlices: number
    /** The number of extra pages allocated for the programs. */
    extraProgramPages: number
  }
}
...
export class TransactionComposer {
  ...
  private async buildMethodCall(
    ...
    const appSizeChange = appId === 0 ? undefined : getAppSizeChange(params)
    ...
    const txnParams = {
      ...
      numGlobalInts: appId === 0 ? ('schema' in params ? (params.schema?.globalInts ?? 0) : 0) : appSizeChange?.numGlobalInts,
      numGlobalByteSlices:
        appId === 0 ? ('schema' in params ? (params.schema?.globalByteSlices ?? 0) : 0) : appSizeChange?.numGlobalByteSlices,
...
/**
 * Returns the size properties to set on an app update transaction, or `undefined` if the update doesn't resize the app
 * (in which case the transaction is built without them and the app keeps its current size).
 */
function getAppSizeChange(
  params: AppCallParams | AppCreateParams | AppUpdateParams | AppCallMethodCall | AppCreateMethodCall | AppUpdateMethodCall,
) {
  const appSize = 'appSize' in params ? params.appSize : undefined
  if (appSize === undefined) return undefined

  return {
    numGlobalInts: appSize.globalInts,
    numGlobalByteSlices: appSize.globalByteSlices,
    extraPages: appSize.extraProgramPages,
  }
}
