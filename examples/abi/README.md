...
All example entrypoints now live one level up (e.g., `../abi-01-type-parsing.ts`). From this directory you can run any example with `npx tsx`:

```bash
npx tsx ../abi-01-type-parsing.ts
npx tsx ../abi-15-arc56-storage.ts
```
