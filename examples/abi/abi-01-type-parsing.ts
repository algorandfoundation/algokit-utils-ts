...
/**
 * Run the ABI type parsing walkthrough used in the docs.
 *
 * @remarks
 * - Shows how to parse primitive, array, tuple, and nested tuple types with `ABIType.from`.
 * - Logs useful properties (`byteLen`, `isDynamic`, `childType`, etc.) and demonstrates equality checks.
 *
 * @example
 * ### Run the example
 * ```bash
 * npx tsx examples/abi/abi-01-type-parsing.ts
 * ```
 
 * {@includeCode ./abi-01-type-parsing.ts}
 */
function main() {
  // #region demo
...
export { main as ABITypeParsingExample }