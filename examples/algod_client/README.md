...
Entrypoints now sit one level up with `algod_client-` prefixes (e.g., `../algod_client-01-node-health-status.ts`). From this directory run with `npx tsx`:

```bash
npx tsx ../algod_client-01-node-health-status.ts
npx tsx ../algod_client-19-sync-round.ts
```
