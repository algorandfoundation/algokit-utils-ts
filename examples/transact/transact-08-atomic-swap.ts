 ...
 * ### Run the example
 * ```bash
 * npx tsx examples/transact/transact-08-atomic-swap.ts
 * ```
 *
 * {@includeCode ./transact-08-atomic-swap.ts}
 */
async function main() {
  ...
  const partyBAlgoDecrease = BigInt(partyBBalanceBefore.microAlgo) - BigInt(partyBBalanceAfter.microAlgo)
  if (partyBAlgoDecrease < swapAlgoAmount) {
    throw new Error(`Party B should have sent at least ${swapAlgoAmount} microALGO`)
  }
  const partyBFeePaid = partyBAlgoDecrease - swapAlgoAmount
  printSuccess(`Party B sent ${formatAlgo(swapAlgoAmount)} (plus ${formatAlgo(partyBFeePaid)} in fees)`)
...
export { main as TransactAtomicSwapExample }